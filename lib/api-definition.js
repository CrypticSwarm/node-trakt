...
api.comment = {
  ...
  "show": { type: "POST", params: [arg("tvdb_id", true), arg("imdb_id", true), arg("title", true), arg("year", true), arg("comment"), arg("spoiler", true), arg("review", true)] }
};
...
api.rate = {
  ...
  "show": { type: "POST", params: [arg("tvdb_id", true), arg("imdb_id", true), arg("title", true), arg("year", true), arg("rating")] },
  "shows": { type: "POST", params: [arg("shows")] }
};
...
// Apply defaults
for (var method_groupname in api) {
  var methods = api[method_groupname];
  for (var method_name in methods) {
    var method = methods[method_name];
    if (method.type === 'GET') {
      method.auth = method.auth == null ? false : method.auth;
    } else {
      method.auth = method.auth == null ? true : method.auth;
    }
