...
trakt.login = function (username, password, callback) {
  ...
  trakt.accountTest(function (err, res) {
    if (res.status === "success") _options.authConfirmed = true;
...
trakt.isLoggedIn = function (confirmed) {
  ...
  return _options.username && _options.password && (_options.authConfirmed || !confirmed);
};
...
function request(rawPath, apiOptions, a, b, backoff) {
  var param, request_obj, auth, reqOptions = {
  ...
  // Make the request
  request_obj = https.request(reqOptions, function (res) {
    var data = "";
    ...
    res.on("end", function () {
      ...
      try {
        ...
        console.log("Bad trakt response backing off", backoff);
        return setTimeout(function () {
          request(rawPath, apiOptions, a, b, backoff * 2);
        }, backoff);
      }
  ...
  request_obj.on("error", function (err) {
    if (callback) callback(err);
  });

  if (apiOptions.type === "POST") request_obj.end(JSON.stringify(params));
  else request_obj.end();
}
